...
foo. XMLExtensible Markup Language
...
(C<xmlns="http://docbook.org/ns/docbook">) on the root element. This
...
namespace.
    ...
    The namespace name foo serves only as an
...
I<DocBook V4.x is feature frozen.>DocBook V4.5 is the last version of
...
I<DocBook V5.0 offers new functionality.> DocBook V5.0 provides
...
I<DocBook V5.0 is more extensible.> Having DocBook V5.0 in a separate
...
I<DocBook V5.0 is easier to customize.> RELAX NG offers many powerful
...
foo (or the mirror at
foo). Only the RELAX NG schema is
...
================================================================================  Basic document structure           YES      YES      YES      YES      YES
  ...
  DatatypesL<^1|In>                       NO       NO       NO      YES      YES

  Co-occurrencesL<^2|RELAX>                  NO       NO       NO      YES      YES

  Hooks for MathML and SVG content    NO       NO       NO      YES      YES

  Link type integrityL<^3|Check>             NO       NO      YES       NO      YES

  Presence of version attribute on    NO       NO      YES       NO      YES
  the root element                                                   


  Miscellaneous checksL<^4|For>            NO       NO      YES       NO      YES

  Element exclusionsL<^5|Prevents>              NO       NO      YES       NO      YES
...
foo. At the time this was written the
...
L<the reference part of DocBook: The Definitive
...
L<nXML mode|http://www.thaiopensource.com/nxml-mode/> is an add-on for
...
=for html <img src="images/emacs.png" alt="Emacs with nXML mode provides guided editing and
validation" />

nXML uses a special configuration file named C<schemas.xml> to associate
schemas with XML documents. Often you will find this file in the
directory C<site-lisp/nxml/schema> inside the Emacs installation
...
associate DocBook V5.0 elements with the appropriate schema:
    ...
    Please note that nXML ships with a file named C<docbook.rnc>. This file
...
If you can't edit the global C<schemas.xml> file, you can create this
...
=for html <img src="images/oxygen5.png" alt="DocBook V5.0 document opened in oXygen in Author
mode" />
...
=for html <img src="images/xxe.png" alt="XML Mind XML Editor – feels almost like MS Word but real DocBook V5.0
markup is created" />
...
foo. It is best to use validators with
...
Validate your document using the following command:
    ...
    The switch C<-Xss512K> increases the stack size of the Java virtual
...
There is also an L<on-line DocBook V5.0
...
You can find a list of NVDL validators at foo. The
...
Modify file C<jnvdl.bat> (or C<jnvdl.sh> on Unix based systems) to include
C<-Xss512K> switch directly after C<java> command.
...
namespace-aware versions from them. The DocBook L<XSL-NS
...
================================================================================  action                    Use C<< remap="action">>.
  ...
  invpartnumber             Use C<< class="other"
  ...
  structfield, structname   Use varname. If you need to distinguish between the two, use C<<
...
To convert your document, C<doc.xml> in the examples below, follow these
steps:
...
Transform C<doc.xml> to C<newdoc.xml> with the C<db4-upgrade.xsl>
...
use a tool that I<is not> XML-aware, or one that allows you to edit
markup “in the raw”.
...
“C<&Product;>” references, you could replace them all with
“C<[[[Product]]]>” (assuming that the string “C<[[[Product]]]>” doesn't
occur anywhere else in your document).

Copy the document type declaration off of your document and save it some
place. The document type declaration is everything from “C<<!DOCTYPE>” to
the closing “C<]>>”.
...
Remove the external identifier (the C<PUBLIC> and/or C<SYSTEM> keywords)
...
The Perl script C<db4-entities.pl>, also included in the DocBook V5.0
distribution attempts to perform this replacement for you. To use the
script, perform the following steps:

Process your document with C<db4-entities.pl>. The script expects a
...
C<db.halign.enumeration> is referenced by the
C<db.videodata.align.enumeration> and C<db.imagedata.align.enumeration>
patterns, which are in turn referenced by the
C<db.videodata.align.attribute> and C<db.imagedata.align.attribute>
...
Each element has a named pattern in the form C<db.>. Elements that
have different content models in different contexts will also have
patterns in the form C<db.>. For example, C<db.figure.info> defines
...
C<db.cals.entrytbl.thead>.



=item -

Most attributes have a named pattern in the form C<db..attribute>.
Attributes that have different content models in different contexts
will also have patterns in the form C<db..attribute>. For example,
C<db.olink.localinfo.attribute> defines the content model of the
...
C<db..attlist> that defines the list of attributes for that element.
Elements that have different attribute lists in different contexts
will also have patterns in the form C<db..attlist> For example,
C<db.html.table.attlist> defines the attribute list for the html
table element and C<db.cals.table.attlist> defines the attribute list
...
form C<db..enumeration>. If the enumeration for a particular
attribute depends on context, optional context is provided. For
example, C<db.verbatim.continuation.enumeration> defines the
...
their context. For example, there is no C<db.class.enumeration>
...
:   Combinations of inline elements, for example,
    C<db.error.inlines>, which contains C<db.errorcode>,
    C<db.errortext>, etc.

blocks
:   Combinations of block elements, for example,
    C<db.verbatim.blocks>, which contains C<db.programlisting>,
    C<db.screen>, etc.

attributes
:   Combinations of attributes, for example,
    C<db.effectivity.attributes>, which contains the attributes arch,
    condition, conformance, etc.

components
:   High level components of the schema, for example,
    C<db.navigation.components>, which contains C<db.glossary>,
    C<db.bibliography>, C<db.index>, and C<db.toc>, and is used inside
    the content model for chapter, appendix, and preface.

contentmodel
:   Shared content models, for example,
    C<db.admonition.contentmodel>, which contains the content model
...
V5.0 schema.L<^6|The> All of the examples are given in both RNG and RNC form.
...
The C<db.author> pattern is copied and renamed C<dbx.person>, defining a
new element called person. Then, the C<db.author> pattern is redefined to
be a choice of the current value or C<dbx.person>. The combine attribute
...
the two elements, creates a new named pattern (C<dbx.asm.inlines>) that
contains them, and adds that pattern to C<db.programming.inlines> and
C<db.os.inlines>. Since these two patterns don't have any elements in
...
content model of C<notAllowed>. Note that in this example, the
...
C<db.person.author.contentmodel>, which defines an author who is a
person, and C<db.org.author.contentmodel>, which defines an author that
is an organization. We will modify C<db.person.author.contentmodel> so
...
born and died to the attribute list for author. The C<db.author.attlist>
...
the C<db.person.author.contentmodel>. The only difference between this
...
use the RELAX NG C<empty> pattern rather than C<notAllowed>. ? deletes the
linking attributes, which are collected in the
C<db.common.linking.attributes> pattern, by defining that pattern as
C<empty>.
...
Generally, C<empty> is used when deleting attributes and C<notAllowed> is
used when deleting elements.


=head3 Changing permitted content of attributes

? modifies C<db.spacing.enumeration> to add the additional value “large”.
...
by using elements in the foo namespace. This
...
using the C<subset> keyword in the description. If your schema contains
any markup model extensions, you can advertise this status by using the
C<extension> keyword. If you'd rather not characterize your variant
specifically as a subset or an extension, use the C<variant> keyword.
...
B<Q:> How do I attach a schema to a DocBook V5.0 document when I do not
want to use DTDs and !DOCTYPE?

B<A:> There is no standard way of associating a RELAX NG schema with a
...
B<A:> Modern schema languages (including RELAX NG and W3X XML Schema)
...
your document. L<Entity definition
...
B<A:> You can use L<XInclude|http://www.w3.org/TR/xinclude/> for this
...
C<docbookxi.rnc> instead of C<docbook.rnc>.

B<Q:> How to validate documents which are composed by XInclude?

B<A:> If you are using XIncludes you should make sure that the final
...
B<Q:> Will the current DocBook XSL stylesheets (XSLT 1.0 based
implementation) be maintained and improved in the future since work on a
new XSLT 2.0 based implementation has started?

B<A:> Yes, the current stylesheets (like 1.73.x) will be supported and
...
B<A:> The basic DocBook schema allows elements from the MathML
...
foo and unpack it somewhere
(e.g. into a C<mathml> subdirectory).
...
Now use the customized schema (C<dbsvg.rnc> or C<dbsvg.rng>) instead of
the original DocBook schema.

B<Q:> Is it possible to use the previous two customizations for MathML
and SVG together?

B<A:> Yes, you can create a special schema customization that combines
...
L<Sample customization of ITS and
...
B<Q:> I'm using Altova XMLSpy to validate DocBook V5.0 instances
against the W3C XML Schema (C<docbook.xsd>). XMLSpy complains about
undefined xml:id attributes?

B<A:> XMLSpy always uses its own bundled version of C<xml.xsd> which
unfortunately doesn't define the xml:id attribute. The bundled version
of C<xml.xsd> is hardwired into the program and cannot be replaced by a
...
March 2003. OASIS. foo

NVDLTUT Nálevka, Petr: . foo

XMLID Marsh, Jonathan – Veillard, Daniel – Walsh, Norman: . W3C
Recommendation, 9 September 2005. foo
